...
static void test_roundtrip_stream(void)
{
    ...
    CHECK(quic_h3conn_send_request(0, qhdrs, sizeof(qhdrs), 0, 0,
    ...
    CHECK(quic_h3conn_send_response(0, 200, body, sizeof(body),
                                    resp, sizeof(resp), &resp_len));
    {
        u16 status = 0;
        const u8 *rbody = 0;
        usz rbody_len = 0;
        CHECK(quic_h3conn_recv_response(resp, resp_len, &status,
                                        &rbody, &rbody_len));
        CHECK(status == 200);
...
static void test_roundtrip_onertt(void)
{
    ...
    CHECK(quic_h3conn_send_response(4, 200, body, sizeof(body),
                                    h3, sizeof(h3), &h3_len));
    /* seal: re-wrap the HTTP/3 bytes as a sealed 1-RTT STREAM packet */
    {
        quic_stream_frame f;
        CHECK(quic_frame_get_stream(h3, h3_len, &f));
        CHECK(quic_appdata_send(&k, &hp, dcid, 4, 1, f.stream_id,
                                f.data, (usz)f.length, f.fin,
    ...
    /* open the 1-RTT packet, then decode the response from its STREAM frame */
    {
        ...
        CHECK(quic_appdata_recv(&k, &hp, pkt, total, 4,
                                &sid, &off, &sdata, &slen, &fin));
        CHECK(quic_appdata_stream_frame(sid, off, sdata, slen, fin,
                                        reframed, sizeof(reframed), &rf_len));
        CHECK(quic_h3conn_recv_response(reframed, rf_len, &status,
                                        &rbody, &rbody_len));
        CHECK(status == 200);
        CHECK(rbody_len == sizeof(body));
