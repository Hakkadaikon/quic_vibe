...
static void test_valid_chain(void)
{
    ...
    usz lens[2] = { sizeof(quic_castore_leaf_der),
                    sizeof(quic_castore_root_der) };
    store_with_root(&s);
...
static void test_tampered_signature_fails(void)
{
    ...
    certs[0] = leaf;             lens[0] = sizeof(leaf);
    certs[1] = quic_castore_root_der; lens[1] = sizeof(quic_castore_root_der);
    store_with_root(&s);
    CHECK(quic_castore_validate_chain(&s, certs, lens, 2) == 0);
}
