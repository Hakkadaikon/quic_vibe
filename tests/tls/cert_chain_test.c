...
static void test_cert_chain_two(void) {
  ...
  CHECK(quic_tls_cert_chain(body, n, &ctx, &ctx_len, e, 4, &count) == 1);
  CHECK(count == 2);
...
static void test_cert_chain_overflow(void) {
  ...
  CHECK(quic_tls_cert_chain(body, n, &ctx, &ctx_len, e, 4, &count) == 0);
}
...
static void test_cert_chain_truncated(void) {
  ...
  CHECK(quic_tls_cert_chain(body, n - 1, &ctx, &ctx_len, e, 4, &count) == 0);
}
