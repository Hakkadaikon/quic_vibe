...
static void test_sigalg_oid(void)
{
    ...
    CHECK(quic_der_oid_equal(oid, len, quic_oid_ecdsa_sha256,
...
static void test_sigalg_matches(void)
{
    quic_tbscert t;
    CHECK(quic_tbscert_parse(quic_x509_golden + 4, 305, &t) == 1);
    CHECK(quic_tbscert_sigalg_matches(&t, quic_oid_ecdsa_sha256,
                                      sizeof(quic_oid_ecdsa_sha256)) == 1);
}
...
static void test_sigalg_mismatch(void)
{
    quic_tbscert t;
    CHECK(quic_tbscert_parse(quic_x509_golden + 4, 305, &t) == 1);
    CHECK(quic_tbscert_sigalg_matches(&t, quic_oid_ec_pubkey,
                                      sizeof(quic_oid_ec_pubkey)) == 0);
}
