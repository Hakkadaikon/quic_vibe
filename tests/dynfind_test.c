...
static void test_find_full(void)
{
    ...
    u64 idx;
    int vm;
    CHECK(quic_qpack_dyn_find(&t, (const u8 *)"b", 1, (const u8 *)"2", 1, &idx, &vm) == 1);
    CHECK(idx == 1 && vm == 1);
}
...
static void test_find_name_only(void)
{
    ...
    u64 idx;
    int vm;
    CHECK(quic_qpack_dyn_find(&t, (const u8 *)"a", 1, (const u8 *)"x", 1, &idx, &vm) == 1);
    CHECK(idx == 0 && vm == 0);
}
...
static void test_find_prefers_full(void)
{
    ...
    u64 idx;
    int vm;
    CHECK(quic_qpack_dyn_find(&t, (const u8 *)"h", 1, (const u8 *)"y", 1, &idx, &vm) == 1);
    CHECK(idx == 1 && vm == 1);
}
...
static void test_find_miss(void)
{
    ...
    u64 idx;
    int vm;
    CHECK(quic_qpack_dyn_find(&t, (const u8 *)"z", 1, (const u8 *)"9", 1, &idx, &vm) == 0);
}
