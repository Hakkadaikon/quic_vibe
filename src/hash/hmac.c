...
static void feed_pad(quic_sha256_ctx *s, const u8 kb[QUIC_SHA256_BLOCK], u8 pad)
{
...
/* inner = H((K^ipad) || msg) */
static void inner_hash(const u8 kb[QUIC_SHA256_BLOCK], const u8 *msg,
                       usz msg_len, u8 out[QUIC_SHA256_DIGEST])
{
    quic_sha256_ctx s;
    quic_sha256_init(&s);
    feed_pad(&s, kb, 0x36);
...
void quic_hmac_sha256(const u8 *key, usz key_len,
