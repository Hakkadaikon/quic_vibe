...
/* RFC 9204 4.5.1.1 */
int quic_qpack_ric_decode(u64 encoded, u64 max_entries, u64 total_inserts,
                          u64 *ric)
{
    if (encoded == 0) { *ric = 0; return 1; }
    return ric_resolve(encoded, 2 * max_entries, total_inserts + max_entries,
                       ric);
}
