...
static const u32 HFIRST[31] = {0,0,0,0,0,0,20,92,248,508,1016,2042,4090,8184,
    16380,32764,65534,131068,262136,524272,1048550,2097116,4194258,8388568,
    16777194,33554412,67108832,134217694,268435426,536870910,1073741820};
static const u16 HIDX[31] = {0,0,0,0,0,0,10,36,68,74,74,79,82,84,90,92,95,95,
    95,95,98,106,119,145,174,186,190,205,224,253,253};
static const u16 HCNT[31] = {0,0,0,0,0,10,26,32,6,0,5,3,2,6,2,3,0,0,0,3,8,13,
...
{
    if (sym > 255 || c->olen >= c->dcap) { c->ok = 0; return; }
    c->out[c->olen++] = (u8)sym;
}
...
static void step_bit(hctx *c, u32 bit)
{
    c->acc = (c->acc << 1) | bit;
    c->nbits++;
    if (c->nbits > HUFF_MAXLEN) { c->ok = 0; return; }
...
{
    for (u32 b = 8; b != 0 && c->ok; b--)
        step_bit(c, (byte >> (b - 1)) & 1u);
}
...
static int decode_ok(const hctx *c)
{
    return c->ok && pad_valid(c);
}

int quic_qpack_huffman_decode(const u8 *src, usz src_len, u8 *dst, usz dcap,
                              usz *out_len)
{
