...
{
    if (!is_settings) { fail(c, QUIC_H3_ERR_MISSING_SETTINGS); return; }
    c->settings_seen = 1;
}

void quic_h3_control_frame(quic_h3_control *c, int is_settings)
{
    if (!c->settings_seen) { first_frame(c, is_settings); return; }
...
{
    if (goaway_increases(c, id)) { fail(c, QUIC_H3_ERR_ID); return; }
    c->goaway_seen = 1;
    c->goaway_limit = id;
}
