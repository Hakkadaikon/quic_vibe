...
typedef struct {
    usz n;
    struct { u64 id; u64 value; } pairs[QUIC_H3_SETTINGS_MAX];
} quic_h3_settings;
...
usz quic_h3_frame_put(u8 *buf, usz cap, u64 type, const u8 *payload, usz payload_len);
usz quic_h3_frame_get(const u8 *buf, usz n, u64 *type, const u8 **payload, u64 *payload_len);
