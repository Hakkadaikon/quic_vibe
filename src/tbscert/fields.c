...
typedef struct { const u8 **p; usz *len; } tbs_slot;
...
static int take_extensions(quic_derseq *c, quic_tbscert *o)
{
    ...
    usz olen, used;
    if (!quic_derseq_next(c, &tag, &outer, &olen)) return 0;
    return quic_der_read(outer, olen, &tag, &o->extensions,
                         &o->extensions_len, &used);
}
...
int quic_tbscert_parse(const u8 *tbs, usz tbs_len, quic_tbscert *out)
{
    ...
    usz vlen;
    quic_derseq c;
    out->version = 0; out->version_len = 0;
    out->extensions = 0; out->extensions_len = 0;
