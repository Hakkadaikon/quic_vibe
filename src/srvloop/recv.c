...
static int recv_initial(quic_server *s, u8 *pkt, usz len,
...
static int recv_handshake(quic_server *s, u8 *pkt, usz len,
                          const u8 **pl, usz *pl_len)
{
...
/* RFC 9001 5.1: open a 1-RTT packet with the peer-direction CLIENT_AP key. */
static int recv_onertt(quic_server *s, u8 *pkt, usz len,
                       const u8 **pl, usz *pl_len)
{
    const quic_initial_keys *k;
    quic_aes128 hp;
    if (!quic_srvloop_open_keys(s, QUIC_LEVEL_ONERTT, &k, &hp))
        return 0;
    return quic_hspkt_onertt_open(k, &hp, pkt, len,
                                  s->sdrv.iscid_len, pl, pl_len);
}
...
static int recv_at_level(quic_server *s, int level, u8 *pkt, usz len,
                         const u8 **pl, usz *pl_len)
{
    static int (*const open_at[])(quic_server *, u8 *, usz,
                                  const u8 **, usz *) = {
        recv_initial,
        recv_handshake,
        recv_onertt,
    };
    return open_at[level](s, pkt, len, pl, pl_len);
}
                      ...
                      const u8 **payload, usz *payload_len)
{
    if (len == 0 || !quic_connrunner_packet_level(dgram[0], level))
        return 0;
    return recv_at_level(s, *level, dgram, len, payload, payload_len);
}
