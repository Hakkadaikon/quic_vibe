...
{
    u32 t0 = rd32le(m), t1 = rd32le(m + 4), t2 = rd32le(m + 8), t3 = rd32le(m + 12);
    st->h[0] += t0 & 0x3ffffff;
...
static void poly_mulmod(poly *st)
{
    ...
    u32 *h = st->h;
    d[0] = (u64)h[0]*r0 + (u64)h[1]*s4 + (u64)h[2]*s3 + (u64)h[3]*s2 + (u64)h[4]*s1;
    d[1] = (u64)h[0]*r1 + (u64)h[1]*r0 + (u64)h[2]*s4 + (u64)h[3]*s3 + (u64)h[4]*s2;
    d[2] = (u64)h[0]*r2 + (u64)h[1]*r1 + (u64)h[2]*r0 + (u64)h[3]*s4 + (u64)h[4]*s3;
    d[3] = (u64)h[0]*r3 + (u64)h[1]*r2 + (u64)h[2]*r1 + (u64)h[3]*r0 + (u64)h[4]*s4;
    d[4] = (u64)h[0]*r4 + (u64)h[1]*r3 + (u64)h[2]*r2 + (u64)h[3]*r1 + (u64)h[4]*r0;
    carry_reduce(st, d);
}
...
{
    for (usz i = 1; i < 5; i++) { h[i] += h[i - 1] >> 26; h[i - 1] &= 0x3ffffff; }
    h[0] += (h[4] >> 26) * 5;
    h[1] += h[0] >> 26;
    h[0] &= 0x3ffffff;
    h[4] &= 0x3ffffff;
...
{
    u32 c = 5;
    for (usz i = 0; i < 5; i++) { g[i] = h[i] + c; c = g[i] >> 26; g[i] &= 0x3ffffff; }
...
static void poly_finish(poly *st, u8 tag[16])
{
    ...
    for (usz i = 0; i < 4; i++) {
        f += (u64)w[i] + st->pad[i];
        tag[4 * i] = (u8)f; tag[4 * i + 1] = (u8)(f >> 8);
        tag[4 * i + 2] = (u8)(f >> 16); tag[4 * i + 3] = (u8)(f >> 24);
        f >>= 32;
    }
}

void quic_poly1305(const u8 key[QUIC_POLY1305_KEY],
                   const u8 *msg, usz len, u8 tag[QUIC_POLY1305_TAG])
{
    poly st;
    poly_init(&st, key);
