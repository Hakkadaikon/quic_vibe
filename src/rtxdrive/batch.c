...
static int batch_one(const quic_rtxbytes *store, u64 pn, u8 *out, usz cap,
                     usz *off)
{
    usz wrote;
    if (!quic_rtxdrive_build(store, pn, out + *off, cap - *off, &wrote))
        return 0;
    *off += wrote;
    return 1;
}

int quic_rtxdrive_batch(const quic_rtxbytes *store, const u64 *lost_pns, usz n,
