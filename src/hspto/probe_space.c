...
{
    if (initial_inflight)
        return QUIC_HSPTO_SPACE_INITIAL;
    if (handshake_inflight)
        return QUIC_HSPTO_SPACE_HANDSHAKE;
    return QUIC_HSPTO_SPACE_APPLICATION;
}

int quic_hspto_probe_space(int initial_inflight, int handshake_inflight,
