...
static int outer_next(const u8 *v, usz n, usz *off, const u8 **start,
                      u8 *tag, const u8 **val, usz *vlen)
{
    usz used;
    if (*off >= n) return 0;
    *start = v + *off;
...
static int take_fields(const u8 *seq, usz seq_len, quic_x509 *out)
{
    ...
    return take_tbs(seq, seq_len, &off, out)
        && take_alg(seq, seq_len, &off, out)
        && take_sig(seq, seq_len, &off, out);
}
...
int quic_x509_parse(const u8 *cert, usz len, quic_x509 *out)
{
    const u8 *seq;
    usz seq_len;
    return outer_seq(cert, len, &seq, &seq_len)
        && take_fields(seq, seq_len, out);
}
