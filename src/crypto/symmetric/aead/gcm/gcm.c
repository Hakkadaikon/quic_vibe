...
static usz ctr_chunk(const quic_aes128 *a, u8 ctr[16],
                     const u8 *in, usz off, usz len, u8 *out)
{
    u8 ks[16];
...
static void ctr_xor(const quic_aes128 *a, u8 ctr[16],
                    const u8 *in, usz len, u8 *out)
{
    usz off = 0;
...
static void gcm_setup(const quic_aes128 *a, const u8 *nonce, u8 h[16], u8 j0[16])
{
    u8 zero[16];
...
static void gcm_tag(const quic_aes128 *a, const u8 h[16], const u8 j0[16],
                    const u8 *aad, usz aad_len, const u8 *ct, usz ct_len,
                    u8 tag[16])
{
...
usz quic_gcm_seal(const quic_aes128 *a, const u8 nonce[QUIC_GCM_NONCE],
                  const u8 *aad, usz aad_len,
                  const u8 *pt, usz pt_len,
                  u8 *ct, u8 tag[QUIC_GCM_TAG])
{
...
int quic_gcm_open(const quic_aes128 *a, const u8 nonce[QUIC_GCM_NONCE],
                  ...
                  const u8 tag[QUIC_GCM_TAG], u8 *pt)
{
