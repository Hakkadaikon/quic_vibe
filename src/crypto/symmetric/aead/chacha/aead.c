...
static void chapoly_tag(const u8 *key, const u8 *nonce,
                        const u8 *aad, usz aad_len,
                        const u8 *ct, usz ct_len, u8 tag[16])
{
...
usz quic_chapoly_seal(const u8 key[QUIC_CHACHA_KEY],
                      const u8 nonce[QUIC_CHACHA_NONCE],
                      const u8 *aad, usz aad_len,
                      const u8 *pt, usz pt_len,
                      u8 *ct, u8 tag[QUIC_CHAPOLY_TAG])
{
...
int quic_chapoly_open(const u8 key[QUIC_CHACHA_KEY],
                      const u8 nonce[QUIC_CHACHA_NONCE],
                      const u8 *aad, usz aad_len,
                      const u8 *ct, usz ct_len,
                      const u8 tag[QUIC_CHAPOLY_TAG], u8 *pt)
{
    u8 want[16];
