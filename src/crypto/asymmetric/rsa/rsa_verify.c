...
/* Write DigestInfo (prefix + hash) at em+off. */
static void put_digestinfo(u8 *em, usz off, const u8 *h, usz h_len) {
  const u8 *prefix = di_prefix(h_len);
  for (usz i = 0; i < RSA_DI_LEN; i++) em[off + i] = prefix[i];
  for (usz i = 0; i < h_len; i++) em[off + RSA_DI_LEN + i] = h[i];
}
...
static int emsa_pkcs1(u8 *em, usz em_len, const u8 *h, usz h_len) {
  usz fixed = 3 + RSA_DI_LEN + h_len; /* 00 01 ...00 prefix H */
...
int quic_rsa_pkcs1_verify(
    ...
    const u8 *msg_hash,
    usz       hash_len) {
  if (!rsa_inputs_ok(n_len, e, e_len, sig_len, hash_len)) return 0;
  quic_bn bn_n, bn_s;
