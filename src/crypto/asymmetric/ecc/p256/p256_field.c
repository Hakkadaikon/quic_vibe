...
void quic_fp_set(p256_fe r, const p256_fe a) { for (usz i = 0; i < 4; i++) r[i] = a[i]; }
...
void quic_fp_add(p256_fe r, const p256_fe a, const p256_fe b, const p256_fe m)
{
    ...
    if (over) fe_sub_raw(r, t, m);
    else quic_fp_set(r, t);
}

void quic_fp_sub(p256_fe r, const p256_fe a, const p256_fe b, const p256_fe m)
{
    if (fe_ge(a, b)) fe_sub_raw(r, a, b);
    else { p256_fe t; fe_sub_raw(t, b, a); fe_sub_raw(r, m, t); }
}
...
/* Subtract m << shift from the 512-bit accumulator w if it fits (w >= m<<shift).
 * shift is a whole bit count 0..256. */
...
void quic_fp_sqr(p256_fe r, const p256_fe a, const p256_fe m) { quic_fp_mul(r, a, a, m); }

void quic_fp_reduce(p256_fe r, const p256_fe a, const p256_fe m)
{
    u64 w[8];
    for (usz i = 0; i < 4; i++) { w[i] = a[i]; w[i + 4] = 0; }
    fe_reduce_wide(r, w, m);
}

/* r = a^e mod m, e given big-endian as the bytes of (m-2) supplied by caller. */
static void fp_pow(p256_fe r, const p256_fe a, const p256_fe e, const p256_fe m)
{
    p256_fe base;
    quic_fp_set(base, a);
    r[0] = 1; r[1] = r[2] = r[3] = 0;
