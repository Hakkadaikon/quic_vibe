...
{
    for (usz i = 0; i < sizeof(peer_control_rules) / sizeof(*peer_control_rules); i++)
        if (peer_control_rules[i].hit(st, ft))
            return peer_control_rules[i].code;
    return 0;
}

/* RFC 9114 6.2.1 / 7.2.4 */
int quic_h3srv_on_peer_control(quic_h3srv_state *st, u64 first_frame_type,
                               u16 *err)
{
