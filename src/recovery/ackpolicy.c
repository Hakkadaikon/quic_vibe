...
{
    if (p->pending == 0) p->since_tick = tick; /* start the delay from the oldest */
    p->pending++;
}
...
int quic_ackpolicy_should_ack(const quic_ackpolicy *p, u64 now, u64 max_ack_delay)
{
